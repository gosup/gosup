...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  context "a new instance" do
    setup do
      @user_friendship = UserFriendship.new user: users(:ben), friend: users(:fred)
    end
  ...
  context "#send_request_email" do
    setup do
      @user_friendship = UserFriendship.create user: users(:ben), friend: users(:fred)
    end
  ...
  context "#mutual_friendship" do
    setup do
      UserFriendship.request users(:ben), users(:fred)
      @friendship1 = users(:ben).user_friendships.where(friend_id: users(:fred).id).first
      @friendship2 = users(:fred).user_friendships.where(friend_id: users(:ben).id).first
    end
  ...
  context "#accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:ben), users(:fred)
    end
  ...
  context "#block!" do
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf = UserFriendship.request users(:ben), users(:fred)
      assert !uf.save
    end
  end
end